// scripts/benchmark/run-ncc-benchmark.ts
...
async function main() {
  ...
  const technique = PackagingTechnique.VERCEL_NCC;
  const resultsDir = process.env.RESULTS_DIR || 'results';

  console.log(`--- Starting ${technique} Benchmarks ---`);

  const apiBenchmark = new PackagingBenchmark('api');
  await runAndRecordBenchmark(apiBenchmark, technique, 'api', 'bun run package:ncc', apiPath);
  ...
  await runAndRecordBenchmark(webBenchmark, technique, 'web', 'bun run package:ncc', webPath);
