// scripts/benchmark/run-pkg-benchmark.ts
...
async function main() {
  ...
  const technique = PackagingTechnique.VERCEL_PKG;
  const resultsDir = process.env.RESULTS_DIR || 'results';

  console.log(`--- Starting ${technique} Benchmarks ---`);

  const apiBenchmark = new PackagingBenchmark('api');
  await runAndRecordBenchmark(apiBenchmark, technique, 'api', 'bun run package:pkg', apiPath);
  ...
  await runAndRecordBenchmark(webBenchmark, technique, 'web', 'bun run package:pkg', webPath);
